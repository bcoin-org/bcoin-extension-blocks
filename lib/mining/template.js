...
function BlockTemplate(options) {
  ...
  this.extCommitment = encoding.ZERO_HASH;
  this.left = DUMMY;
  this.right = DUMMY;
  this.items = [];
  this.ext = [];
...
/**
 * Create extension commitment hash.
 * @returns {Buffer}
 */

BlockTemplate.prototype.getExtensionHash = function getExtensionHash() {
  var leaves = [];
  var i, item, root;
...
BlockTemplate.prototype.getExtensionScript = function getExtensionScript() {
  return Script.fromExtensionHash(this.extCommitment);
};
...
BlockTemplate.prototype.getWitnessHash = function getWitnessHash(res) {
 ...
 * @param {Buffer} extHash - Extension commitment hash.
 * @returns {TX}
 */

BlockTemplate.prototype.createCoinbase = function createCoinbase(hash, extHash) {
  ...
  // If we're using extblk, we
  // need to set up the commitment.
  if (this.extended) {
    // Commitment output.
    commit = new Output();
    commit.script.fromExtensionHash(extHash);
    cb.outputs.push(commit);
  }
...
BlockTemplate.prototype.refresh = function refresh() {
  ...
  var hash = this.getWitnessHash(res);
  var extHash = this.getExtensionHash();
  var cb = this.createCoinbase(hash, extHash);
  var raw = cb.toNormal();
  var size = 0;
  var left, right;
...
BlockTemplate.prototype.addTX = function addTX(tx, view) {
  var scale = consensus.WITNESS_SCALE_FACTOR;
  var item, weight, extSize, sigops, extCost, extSigops;
  ...
  if (item.extended) {
    ...
    if (this.extSigops + extSigops > consensus.MAX_EXTENSION_SIGOPS_COST)
      return false;
...
BlockTemplate.prototype.pushTX = function pushTX(tx, view) {
  ...
  if (item.extended) {
    this.extSize += tx.getSize();
    this.extSigops += item.sigops;
    this.extFees += item.fee;
    this.cost += item.cost;
    this.weight += tx.getExitSize() * scale;
    this.sigops += tx.getExitSigops() * scale;
  } else {
    this.weight += tx.getWeight();
    this.weight += tx.getEnterSize() * scale;
    this.sigops += item.sigops;
    this.fees += item.fee;
  }
...
BlockEntry.fromTX = function fromTX(tx, view, attempt) {
  ...
  if (item.extended) {
    ...
    item.sigops = tx.getExtensionSigops(view);
  } else {
