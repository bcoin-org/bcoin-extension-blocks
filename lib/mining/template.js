...
BlockTemplate.prototype.getCommitmentHash = function getCommitmentHash() {
  var leaves = [];
  var i, item, root;
...
BlockTemplate.prototype.refresh = function refresh() {
  var hash = this.getCommitmentHash();
...
BlockTemplate.prototype.addTX = function addTX(tx, view) {
  ...
  if (item.witness) {
    ...
    if (this.extSize + extSize > consensus.MAX_BLOCK_SIZE)
      return false;

    if (this.extSigops + sigops > consensus.MAX_BLOCK_SIGOPS_COST)
      return false;

    this.size += size;
    this.extSize += extSize;
    this.extSigops += sigops;
...
BlockEntry.fromTX = function fromTX(tx, view, attempt) {
  ...
  item.witness = tx.isExtension(view);
  return item;
};
