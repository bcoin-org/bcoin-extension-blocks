...
Miner.prototype.assemble = function assemble(attempt) {
  ...
  var prev, deps, hashes, size, extSize, sigops, block, resSize;
  ...
  while (queue.size() > 0) {
    ...
    resSize = attempt.resSize;

    if (!tx.isFinal(attempt.height, attempt.locktime))
      continue;

    if (item.witness) {
      sigops = attempt.extSigops;
      ...
      if (extSize > this.options.maxSize)
        continue;
      ...
      if (priority) {
        if (extSize > this.options.prioritySize
        ...
        if (item.free && extSize >= this.options.minSize)
          continue;
      }

      attempt.size = size;
      attempt.extSize = extSize;
      attempt.extSigops = sigops;
  ...
  assert(attempt.extSize <= consensus.MAX_BLOCK_SIZE,
  ...
  if (this.options.preverify) {
    ...
    assert(block.getExtensionSize() <= consensus.MAX_BLOCK_SIZE,
      'Block exceeds max block size.');
  }
};
