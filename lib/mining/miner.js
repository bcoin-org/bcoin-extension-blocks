...
Miner.prototype._createBlock = co(function* createBlock(tip, address) {
  ...
  this.logger.debug(
    'Created block template'
    + ' (height=%d, weight=%d+%d, cost=%d, fees=%d+%d, txs=%d+%d, diff=%d).',
    attempt.height,
    attempt.weight,
    attempt.extSize,
    attempt.extCost,
    Amount.btc(attempt.fees),
    Amount.btc(attempt.extFees),
    attempt.items.length + 1 + (attempt.resolution ? 1 : 0),
    attempt.ext.length,
...
Miner.prototype.getAddress = function getAddress() {
  ...
  if (addr.version >= 9)
    throw new Error('Cannot mine with an extension address.');
...
Miner.prototype.assemble = function assemble(attempt) {
  ...
  if (this.options.preverify) {
    ...
    // Explanation for the extra 15:
    // Potential varints for:
    // - extblk tx count.
    // - resolution input count
    // - resolution output count
    // assert(block.getSize() <= attempt.weight + attempt.extSize + 15,
    //   'Block exceeds reserved weight!');

    // assert(block.getBaseSize() <= attempt.weight + 10,
    //   'Block exceeds reserved weight!');

    // assert(block.getExtensionSize() <= attempt.extSize + 5,
    //   'Block exceeds reserved weight!');

    // assert(block.getBaseSize() <= consensus.MAX_BLOCK_SIZE,
    //   'Block exceeds max block weight.');
...
MinerOptions.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.extMaxSigops != null) {
    ...
    assert(options.extMaxSigops <= consensus.MAX_EXTENSION_SIGOPS_COST,
      'Max sigops must be below MAX_EXTENSION_EXTENSION_COST');
