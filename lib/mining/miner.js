...
Miner.prototype.assemble = function assemble(attempt) {
  ...
  var prev, deps, hashes, size, extSize, extSigops, sigops, block;
  ...
  while (queue.size() > 0) {
    ...
    if (item.extended) {
      ...
      if (extSize > this.options.maxSize * 10)
        continue;

      extSigops += item.sigops;

      if (extSigops > this.options.maxSigops)
        continue;
  ...
  assert(attempt.extSize <= consensus.MAX_BLOCK_SIZE * 10,
  ...
  if (this.options.preverify) {
    ...
    assert(block.getExtensionSize() <= consensus.MAX_BLOCK_SIZE * 10,
      'Block exceeds max block size.');
  }
};
