...
WalletDB.prototype._open = co(function* open() {
  ...
  yield this.db.open();
  yield this.db.checkVersion('V', 6 | 0x80);
...
WalletDB.prototype.bind = function bind() {
  ...
  this.client.on('block connect', co(function* (entry, txs, ext) {
    try {
      yield self.addBlock(entry, txs, ext);
    } catch (e) {
      self.emit('error', e);
    }
  }));
  ...
  this.client.hook('block rescan', co(function* (entry, txs, ext) {
    try {
      yield self.rescanBlock(entry, txs, ext);
    } catch (e) {
      self.emit('error', e);
    }
  }));
...
WalletDB.prototype.addBlock = co(function* addBlock(entry, txs, ext) {
  var unlock = yield this.txLock.lock();
  try {
    return yield this._addBlock(entry, txs, ext);
  } finally {
    unlock();
  }
});
...
WalletDB.prototype._addBlock = co(function* addBlock(entry, txs, ext) {
...
WalletDB.prototype.rescanBlock = co(function* rescanBlock(entry, txs, ext) {
  if (!this.rescanning) {
  ...
  try {
    yield this._addBlock(entry, txs, ext);
  } catch (e) {
    this.emit('error', e);
    throw e;
  }
