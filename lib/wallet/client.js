...
WalletClient.prototype._open = co(function* _open() {
  ...
  this.socket.on('block connect', function(entry, txs, ext) {
    var block;

    try {
      block = parseBlock(entry, txs, ext);
    } catch (e) {
      self.emit('error', e);
      return;
    }

    self.emit('block connect', block.entry, block.txs, block.ext);
  });
  ...
  this.socket.on('block rescan', function(entry, txs, ext, cb) {
    var block;
...
function parseBlock(entry, txs, ext) {
  ...
  return new BlockResult(block, txv, extv);
}
...
function BlockResult(entry, txs, ext) {
  this.entry = entry;
  this.txs = txs;
  this.ext = ext;
}
