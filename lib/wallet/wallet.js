...
Wallet.prototype._fund = co(function* fund(mtx, options) {
  var rate, account, coins, extension;
  ...
  if (extension == null && options.account != null)
    extension = account.witness;
...
Wallet.prototype._send = co(function* send(options, passphrase) {
  ...
  if (tx.getExtensionCost(mtx.view) > policy.MAX_TX_COST)
    throw new Error('TX exceeds policy cost.');
...
Wallet.prototype.syncOutputDepth = co(function* syncOutputDepth(details) {
  ...
  for (i = 0; i < accounts.length; i++) {
    ...
    ring = yield account.syncDepth(receive, change);
