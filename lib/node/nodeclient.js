...
NodeClient.prototype._init = function init() {
  ...
  this.node.on('connect', function(entry, block) {
    ...
    self.emit('block connect', entry, block.txs, block.ext);
  });
...
NodeClient.prototype.rescan = function rescan(start) {
  var self = this;
  return this.node.chain.scan(start, this.filter, function(entry, txs, ext) {
    return self.fire('block rescan', entry, txs, ext);
  });
};
