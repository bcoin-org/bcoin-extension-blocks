...
Pool.prototype.isSyncable = function isSyncable(peer) {
  ...
  if (this.options.hasExtension() && !peer.hasExtension())
    return false;
...
Pool.prototype.getBroadcasted = function getBroadcasted(peer, item) {
  ...
  this.logger.debug(
    ...
    item.isTX() ? 'tx' : 'block',
    item.rhash(),
    item.hasExtension() ? 'extension' : 'normal',
    peer.hostname());
...
Pool.prototype.sendBlock = co(function* sendBlock(peer, item, extension) {
  var block = this.getBroadcasted(peer, item);

  // Check for a broadcasted item first.
  if (block) {
    peer.send(new packets.BlockPacket(block, extension));
    return true;
  }
  ...
  if (extension || !this.options.hasExtension()) {
...
Pool.prototype.handleGetData = co(function* handleGetData(peer, packet) {
  ...
  for (i = 0; i < items.length; i++) {
    ...
    if (item.isTX()) {
      ...
      peer.send(new packets.TXPacket(tx, item.hasExtension()));
    ...
    switch (item.type) {
      case invTypes.BLOCK:
      case invTypes.EXTENSION_BLOCK:
        result = yield this.sendBlock(peer, item, item.hasExtension());
        if (!result) {
          notFound.push(item);
          continue;
        }
      ...
      case invTypes.CMPCT_BLOCK:
        ...
        if (height < this.chain.tip.height - 10) {
          result = yield this.sendBlock(peer, item, peer.compactExtension);
          if (!result) {
            notFound.push(item);
            continue;
          }
...
Pool.prototype.handleCmpctBlock = co(function* handleCmpctBlock(peer, packet) {
  var block = packet.block;
  var hash = block.hash('hex');
  var extension = peer.compactExtension;
  ...
  result = block.fillMempool(extension, this.mempool);
...
Pool.prototype.handleGetBlockTxn = co(function* handleGetBlockTxn(peer, packet) {
  ...
  peer.send(new packets.BlockTxnPacket(res, peer.compactExtension));
});
...
PoolOptions.prototype.getRequiredServices = function getRequiredServices() {
  ...
  if (this.hasExtension())
    services |= common.services.EXTENSION;
  return services;
};
...
PoolOptions.prototype.hasExtension = function hasExtension() {
  return this.chain.state.hasExtension();
};
