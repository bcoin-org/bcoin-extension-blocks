...
Peer.prototype.sendCompactBlock = function sendCompactBlock(block) {
  var extension = this.compactExtension;
  var compact = BIP152.CompactBlock.fromBlock(block, extension);
  this.send(new packets.CmpctBlockPacket(compact, extension));
};
 ...
 * compact, extension, or non-extension).
 * @returns {Number}
 */
...
Peer.prototype.blockType = function blockType() {
  ...
  if (this.hasExtension())
    return invTypes.EXTENSION_BLOCK;

  return invTypes.BLOCK;
};

/**
 * Calculate peer tx inv type (extension or non-extension).
 * @returns {Number}
 */

Peer.prototype.txType = function txType() {
  if (this.hasExtension())
    return invTypes.EXTENSION_TX;
...
Peer.prototype.handleVersion = co(function* handleVersion(packet) {
  ...
  if (this.outbound) {
    ...
    if (this.options.hasExtension()) {
      if (!(this.services & services.EXTENSION))
        throw new Error('Peer does not support extension blocks.');
    }
...
Peer.prototype.handleSendCmpct = co(function* handleSendCmpct(packet) {
  ...
  if (packet.version > 3 || packet.version === 2) {
    // Ignore
    this.logger.info(
  ...
  this.compactExtension = packet.version === 3;
});
...
Peer.prototype.sendCompact = function sendCompact(mode) {
  if (this.services & common.services.EXTENSION) {
    if (this.version >= common.COMPACT_EXTENSION_VERSION) {
      this.logger.info(
        'Initializing extension compact blocks (%s).',
        this.hostname());
      this.send(new packets.SendCmpctPacket(mode, 3));
      return;
    }
  }
...
/**
 * Test whether the EXTENSION service bit is set.
 * @returns {Boolean}
 */

Peer.prototype.hasExtension = function hasExtension() {
  return (this.services & services.EXTENSION) !== 0;
};
...
Peer.prototype.hasCompactSupport = function hasCompactSupport() {
  ...
  return this.version >= common.COMPACT_EXTENSION_VERSION;
};
...
function PeerOptions(options) {
  ...
  this.hasExtension = PeerOptions.hasExtension;
...
PeerOptions.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.hasExtension != null) {
    assert(typeof options.hasExtension === 'function');
    this.hasExtension = options.hasExtension;
  }
...
PeerOptions.hasExtension = function hasExtension() {
  return true;
};
