...
Chain.prototype.verify = co(function* verify(block, prev, flags) {
  ...
  // The canonical block cannot
  // contain witness vectors.
  if (block.hasWitness()) {
    throw new VerifyError(block,
      'invalid',
      'unexpected-witness',
  ...
  if (!commit) {
    ...
    // - The resolution transaction is not the
    //   genesis resolution.
    // OR:
    ...
    // outputs.
    if (prev.resHash !== encoding.NULL_HASH || block.hasProgram()) {
  ...
  // Check extension block size limit (not the
  // real DoS limit we should be focusing on).
  if (block.getExtensionSize() > consensus.MAX_BLOCK_SIZE * 100) {
    throw new VerifyError(block,
      'invalid',
      'bad-blk-length',
      100);
...
Chain.prototype.verifyInputs = co(function* verifyInputs(block, prev, state) {
  ...
  var genesis = false;
  var flags = state.flags & ~Script.flags.VERIFY_WITNESS;
  ...
  var jobs = [];
  var index = 1;
  var i, j, output, tx, valid, fee, coins;
  ...
  for (i = 0; i < block.txs.length; i++) {
    ...
    if (resolution)
      genesis = prev.resHash === encoding.NULL_HASH;

    // Ensure tx is not double spending an output.
    if (i > 0) {
      if (!(yield view.spendInputs(this.db, tx, resolution, genesis))) {
    ...
    if (i > 0) {
      fee = tx.checkContext(view, height, resolution, genesis, ret);
      ...
      if (state.hasWitness() && !resolution && !tx.hasCanonical(view)) {
        throw new VerifyError(block,
          'invalid',
          'bad-program-redemption',
          100);
      }

      // Push onto verification queue.
      jobs.push(tx.verifyAsync(view, flags));
    }
  ...
  for (i = 0; i < block.ext.length; i++) {
    ...
    if (!tx.hasPrograms(view)) {
      throw new VerifyError(block,
        'invalid',
        'bad-extension-redemption',
        100);
    }

    // Ensure tx is not double spending an output.
    if (!(yield view.spendInputs(this.db, tx, false, false))) {
      assert(!historical, 'BUG: Spent inputs in historical data!');
      throw new VerifyError(block,
        'invalid',
        'bad-txns-inputs-missingorspent',
    ...
    // Contextual sanity checks.
    fee = tx.checkContext(view, height, false, false, ret);
    ...
    for (j = 0; j < tx.outputs.length; j++) {
      ...
      if (!output.script.isProgram()) {
        ...
        if (!util.equal(output.script.raw, res.outputs[index].script.raw)) {
          throw new VerifyError(block,
            'invalid',
        ...
        if (output.value !== res.outputs[index].value) {
          throw new VerifyError(block,
            'invalid',
  ...
  if (resolution) {
    ...
    // Fees propagate up from the extension
    // block to the resoluton transaction.
    if (res.getOutputValue() !== res.getInputValue(view) - fees) {
      throw new VerifyError(block,
        'invalid',
        'bad-resolution-fees',
        100);
