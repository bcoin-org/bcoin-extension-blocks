...
Chain.prototype.verify = co(function* verify(block, prev, flags) {
  ...
  // Check the commitment hash for extblk.
  if (state.hasExtension()) {
    commit = block.getExtensionHash();
    if (commit) {
      hash = block.createExtensionHash();
...
Chain.prototype.verifyCanonical = co(function* verifyCanonical(block, prev, view, state) {
  ...
  var extension = state.hasExtension();
  var sigops = 0;
  var reward = 0;
  var jobs = [];
  var i, tx, valid, fee;
  ...
  for (i = 0; i < block.txs.length; i++) {
    ...
    // Stop at the resolution.
    if (extension && i === block.txs.length - 1) {
      if (tx.isResolution()) {
        if (!(yield view.ensureInputs(this.db, tx))) {
    ...
    // Only the resolution transaction
    // may use a uint32_max version.
    if (extension) {
      if (tx.isResolution()) {
        throw new VerifyError(block,
          'invalid',
    ...
    if (i > 0) {
      ...
      if (extension) {
        // Entering outputs are also bound
        // by the consensus dust rule.
        if (tx.hasDust()) {
          throw new VerifyError(block,
            'invalid',
            'dust',
...
Chain.prototype.verifyExtension = co(function* verifyExtension(block, prev, view, state) {
  ...
  var fees = 0;
  var jobs = [];
  var i, input, output, tx, valid, fee;
  ...
  if (value !== -1) {
    ...
    // Load previous resolution output.
    assert(yield view.ensureInputs(this.db, res));

    // Sanity checks.
    assert(res.verify(view, state.flags));
    assert(res.getInputValue(view) >= res.getOutputValue());

    if (prev.resHash === encoding.NULL_HASH) {
      // The first input enters normally if we're
      // the genesis resolution transaction.
      view.enter(input);
    } else {
      // Spend the last resolution output.
      view.spend(input);
    }

    // Transfer the entering outputs
    // to the extension block chain.
  ...
  for (i = 0; i < block.ext.length; i++) {
    ...
    // Verify sequence locks.
    if (tx.version >= 2) {
      valid = yield this.verifyLocks(prev, tx, view, state.extLockFlags);
    ...
    if (sigops > consensus.MAX_EXTENSION_SIGOPS_COST) {
      throw new VerifyError(block,
        'invalid',
        'bad-blk-sigops',
        100);
  ...
  if (value !== -1) {
    ...
    // Add new coins for resolution.
    view.exit(res, height);

    // Mark the new resolution.
    view.resHash = res.hash('hex');
    view.resValue = output.value;
  }
...
Chain.prototype.verifyResolution = function verifyResolution(block, prev) {
  ...
  for (i = 0; i < block.txs.length; i++) {
    ...
    // All entering inputs should be
    // swept by the resolution transaction.
    for (j = 0; j < tx.outputs.length; j++) {
      ...
      if (inputIndex >= res.inputs.length) {
        throw new VerifyError(block,
          'invalid',
      ...
      target = res.inputs[inputIndex];

      if (target.prevout.hash !== hash) {
        throw new VerifyError(block,
          'invalid',
      ...
      if (inputIndex === 0 && target.script.raw.length > 0) {
        // Genesis resolution gets a special message.
        if (target.script.raw.length > 100
            || target.script.code.length > 1
            || !target.script.isPushOnly()
            || target.script.code[0].value === opcodes.OP_RESERVED) {
          throw new VerifyError(block,
            'invalid',
      ...
      assert(target.witness.items.length === 0);

      if (output.value < 0 || output.value > consensus.MAX_MONEY) {
        throw new VerifyError(block,
          'invalid',
          'bad-txns-outputvalues-outofrange',
          100);
      }
  ...
  for (i = 0; i < block.ext.length; i++) {
    ...
    for (j = 0; j < tx.outputs.length; j++) {
      ...
      if (outputIndex >= res.outputs.length) {
        throw new VerifyError(block,
          'invalid',
      ...
      target = res.outputs[outputIndex];

      if (!util.equal(target.script.raw, output.script.raw)) {
        throw new VerifyError(block,
          'invalid',
  ...
  // We do not need a resolution transaction.
  if (!hasEnter && !hasExit && !res.isResolution())
    return -1;
  ...
  // Any non-genesis resolution must
  // redeem the previous resolution output.
  if (prev.resHash !== encoding.NULL_HASH) {
...
Chain.prototype.finish = function finish(block, entry) {
  ...
  this.logger.info(
    'Block %s (%d) added to chain (size=%d+%d txs=%d+%d time=%d).',
    entry.rhash(),
    entry.height,
    block.getSize(),
    block.getExtensionSize(),
    block.txs.length,
    block.ext.length,
    elapsed);
...
function DeploymentState() {
  ...
  this.extLockFLags = Script.flags.STANDARD_LOCKTIME_FLAGS;
