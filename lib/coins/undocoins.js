...
UndoCoins.prototype.push = function push(coins, entry) {
  var undo = new UndoCoin();
  undo.entry = entry;
  undo.extended = entry.extended;
...
UndoCoins.prototype.apply = function apply(view, outpoint) {
  ...
  if (undo.height !== -1) {
    ...
    coins.resolution = undo.resolution;
  } else {
    coins = view.map[hash];
    assert(coins);
  }

  coins.addOutput(index, undo.toOutput(), undo.extended);
...
UndoCoin.prototype.getSize = function getSize() {
  ...
  size += encoding.sizeVarint(height * 4
...
UndoCoin.prototype.fromReader = function fromReader(br) {
  ...
  this.height = code / 4 | 0;

  if (this.height === 0)
    this.height = -1;

  this.resolution = (code & 2) !== 0;
