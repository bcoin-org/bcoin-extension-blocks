...
function CoinView() {
  ...
  this.resHash = encoding.NULL_HASH;
  this.resValue = 0;
}
...
CoinView.prototype.addTX = function addTX(tx, height, extended) {
  var coins = Coins.fromTX(tx, height, extended);
...
CoinView.prototype.removeTX = function removeTX(tx, height) {
  var coins = Coins.fromTX(tx, height, false);
...
CoinView.prototype.addCoin = function addCoin(coin) {
  ...
  if (!coins) {
    ...
    coins.resolution = coin.resolution;
    coins.extended = coin.extended;
    this.add(coins);
  }
...
CoinView.prototype.addOutput = function addOutput(hash, index, output, extended) {
  ...
  if (!coins) {
    ...
    coins.resolution = false;
    coins.extended = !!extended;
    this.add(coins);
  }
  ...
  if (!coins.has(index))
    coins.addOutput(index, output, extended);
};
...
/**
 * Spend an output (by input).
 * @param {Input} input
 * @returns {Boolean}
 */

CoinView.prototype.spend = function spend(input) {
  var coins = this.get(input.prevout.hash);

  if (!coins)
    return false;

  return this.spendFrom(coins, input.prevout.index);
};
...
CoinView.prototype.spendFrom = function spendFrom(coins, index) {
  ...
  if (coins.isEmpty()) {
    ...
    undo.resolution = coins.resolution;
...
/**
 * Test whether an output is on the ext. block chain.
 * @param {Input} input
 * @returns {Boolean}
 */

CoinView.prototype.isExtension = function isExtension(input) {
  var coins = this.get(input.prevout.hash);
  var entry;
...
/**
 * Transfer an input from the canonical
 * chain to the extension chain.
 * @param {Input} input
 */

CoinView.prototype.enter = function enter(input) {
  var coins = this.get(input.prevout.hash);
  var entry;

  assert(coins);
  assert(this.spendFrom(coins, input.prevout.index));
...
/**
 * Transfer an input from the extension
 * chain to the canonical chain.
 * @param {TX} res
 * @param {Number} height
 */

CoinView.prototype.exit = function exit(res, height) {
  var coins = this.addTX(res, height, false);
  coins.resolution = true;
  return coins;
};
