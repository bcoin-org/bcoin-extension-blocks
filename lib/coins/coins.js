...
Coins.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.resolution != null) {
    assert(typeof options.resolution === 'boolean');
...
Coins.prototype.addOutput = function addOutput(index, output, extended) {
  assert(!output.script.isUnspendable());
  this.add(index, CoinEntry.fromOutput(output, extended));
};
...
Coins.prototype.addCoin = function addCoin(coin, extended) {
  assert(!coin.script.isUnspendable());
  this.add(coin.index, CoinEntry.fromCoin(coin, extended));
};
...
/**
 * Test whether the collection
 * has an alt chain coin.
 * @param {Number} index
 * @returns {Boolean}
 */

Coins.prototype.isExtension = function isExtension(index) {
  var output;
...
Coins.prototype.toRaw = function toRaw() {
  ...
  bw.writeU8(this.resolution ? 1 : 0);
  bw.writeVarint(code);

  // Write the spent field.
  for (i = 0; i < size; i++) {
    ch = 0;
    for (j = 0; j < 4 && 2 + i * 4 + j < len; j++) {
      if (this.isUnspent(2 + i * 4 + j)) {
        ch |= 1 << j;
        if (this.isExtension(2 + i * 4 + j))
          ch |= 2 << j;
      }
    }
    bw.writeU8(ch);
...
Coins.prototype.fromRaw = function fromRaw(data, hash) {
  ...
  if ((code & 2) !== 0) {
    ...
    first.extended = (code & 8) !== 0;
  }

  if ((code & 4) !== 0) {
    second = CoinEntry.fromReader(br);
    second.extended = (code & 16) !== 0;
  }
  ...
  for (i = 0; i < size; i++) {
    ...
    for (j = 0; j < 8; j += 2) {
      ...
      output = CoinEntry.fromReader(br);
      output.extended = (ch & (2 << j)) !== 0;
...
Coins.prototype.fromTX = function fromTX(tx, height, extended) {
  var i, output, entry;

  assert(typeof height === 'number');
  assert(typeof extended === 'boolean');
  ...
  for (i = 0; i < tx.outputs.length; i++) {
    ...
    // Do not add exiting outputs to the
    ...
    entry = CoinEntry.fromOutput(output, extended);
...
Coins.fromTX = function fromTX(tx, height, extended) {
  return new Coins().fromTX(tx, height, extended);
};
...
CoinEntry.fromOutput = function fromOutput(output, extended) {
  var entry = new CoinEntry();
  entry.output = output;
  assert(typeof extended === 'boolean');
...
CoinEntry.fromCoin = function fromCoin(coin, extended) {
  var entry = new CoinEntry();
