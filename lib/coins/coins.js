...
Coins.prototype.fromTX = function fromTX(tx, height, extension) {
  var i, output;
  ...
  for (i = 0; i < tx.outputs.length; i++) {
    ...
    if (extension && !output.script.isProgram()) {
      this.outputs.push(null);
      continue;
    }
...
Coins.fromTX = function fromTX(tx, height, extension) {
  return new Coins().fromTX(tx, height, extension);
};
