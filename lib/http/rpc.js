...
RPC.prototype._createTemplate = co(function* _createTemplate(maxVersion, coinbase, rules) {
  ...
  // Build an index of every ext. transaction.
  index = {};
  for (i = 0; i < attempt.ext.length; i++) {
    entry = attempt.ext[i];
    index[entry.hash] = i;
  }

  // Calculate dependencies for each ext. transaction.
  for (i = 0; i < attempt.ext.length; i++) {
    entry = attempt.ext[i];
    tx = entry.tx;
    deps = [];
  ...
  json = {
    ...
    extsizelimit: undefined,
    extcostlimit: undefined,
    ...
    default_extension_commitment: undefined,
    transactions: txs,
    extension: ext
  };

  if (attempt.extended) {
    json.extsizelimit = consensus.MAX_EXTENSION_SIZE;
    json.extcostlimit = consensus.MAX_EXTENSION_COST;
  }
  ...
  if (rules && rules.indexOf('extblk') !== -1) {
    commit = attempt.getCommitmentScript();
    if (compat)
      json.default_witness_commitment = commit.toJSON();
    else
      json.default_extension_commitment = commit.toJSON();
  }
...
RPC.prototype.generateToAddress = co(function* _generateToAddress(args, help) {
  ...
  if (addr.isProgram()) {
    throw new RPCError(errs.INVALID_ADDRESS_OR_KEY,
      'Cannot mine with a witness address.');
  }
...
RPC.prototype.getTemplate = co(function* getTemplate() {
  ...
  if (attempt) {
    ...
    if (attempt.extended) {
      hash = attempt.commitment.toString('hex');
      this.commitMap[hash] = attempt.ext;
    }
  }
...
RPC.prototype._addBlock = co(function* _addBlock(block) {
  var i, ext, item, entry;

  this.logger.info('Handling submitted block: %s.', block.rhash());

  ext = yield this._getExtension(block);

  if (ext) {
    this.logger.warning('Submitted block had no extension.');
    this.logger.warning('Injecting extension block.');
...
RPC.prototype._getExtension = co(function* _getExtension(block) {
  var prev, state, hash;

  if (block.hasExtension())
    return;
  ...
  hash = block.getCommitmentHash('hex');

  if (!hash)
    return;

  return this.commitMap[hash];
});
...
RPC.prototype.blockToJSON = co(function* blockToJSON(entry, block, details) {
  ...
  return {
    ...
    size: block.getSize(),
    extsize: block.getExtensionSize(),
    height: entry.height,
    version: entry.version,
    versionHex: util.hex32(entry.version),
    merkleroot: util.revHex(entry.merkleRoot),
