...
HTTPServer.prototype.bindChain = function bindChain() {
  ...
  this.chain.on('connect', function(entry, block, view) {
    ...
    for (item = list.head; item; item = item.next) {
      socket = item.value;
      txs = self.filterBlock(socket, block.txs);
      ext = self.filterBlock(socket, block.ext);
      socket.emit('block connect', raw, txs, ext);
    }
  });
...
HTTPServer.prototype.filterBlock = function filterBlock(socket, txs) {
  var result = [];
  var i, tx;
...
HTTPServer.prototype.scanner = function scanner(socket, entry, txs, ext) {
  ...
  for (i = 0; i < ext.length; i++) {
    tx = ext[i];
    rawExt.push(tx.toRaw());
  }

  socket.emit('block rescan', block, raw, rawExt);
