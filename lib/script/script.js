...
/**
 * Inject properties from an extension block commitment.
 * @private
 * @param {Buffer} hash
 * @param {String|Buffer} flags
 */

Script.prototype.fromExtensionHash = function fromExtensionHash(hash, flags) {
  var bw = new StaticWriter(36);

  bw.writeU32BE(0x45424c4b);
  bw.writeHash(hash);
...
/**
 * Create an extension block commitment.
 * @param {Buffer} hash
 * @param {String|Buffer} flags
 * @returns {Script}
 */

Script.fromExtensionHash = function fromExtensionHash(hash, flags) {
  return new Script().fromExtensionHash(hash, flags);
};
...
Script.prototype.isExtensionHash = function isExtensionHash() {
  return this.raw.length >= 38
    && this.raw[0] === opcodes.OP_RETURN
    && this.raw[1] === 0x24
    && this.raw.readUInt32BE(2, true) === 0x45424c4b;
};
...
Script.prototype.getExtensionHash = function getExtensionHash() {
  if (!this.isExtensionHash())
    return;
...
/**
 * Test whether the output script is a resolution output.
 * @returns {Boolean}
 */

Script.prototype.isResolution = function isResolution() {
  return this.raw.length === 1 && this.raw[0] === opcodes.OP_TRUE;
};
 ...
 * a canonical witness program.
 * @return {Boolean}
 */

Script.prototype.isCanonical = function isCanonical() {
  if (!this.isProgram())
    return false;
  ...
  return this.raw[0] >= opcodes.OP_1 && this.raw[0] <= opcodes.OP_8;
};

/**
 * Test whether the output script is
 * an extension witness program.
 * @return {Boolean}
 */

Script.prototype.isExtension = function isExtension() {
  if (!this.isProgram())
    return false;

  return this.raw[0] >= opcodes.OP_9 && this.raw[0] <= opcodes.OP_16;
};
...
Script.prototype.isWitnessPubkeyhash = function isWitnessPubkeyhash() {
  return this.raw.length === 22
    && (this.raw[0] === opcodes.OP_0 || this.raw[0] === opcodes.OP_9)
    && this.raw[1] === 0x14;
};
...
/**
 * Count the sigops cost for extended transaction inputs.
 * @param {Witness} witness
 * @returns {Number} input cost
 */

Script.prototype.getExtensionSigops = function getExtensionSigops(witness) {
  var factor = consensus.WITNESS_V9_FACTOR;
  var program, redeem;
  ...
  if (program.version === 9) {
    ...
    if (program.data.length === 32) {
      if (witness.items.length === 0)
        return 0;
      redeem = witness.getRedeem();
      return redeem.getSigops(true) * factor;
    }
  }

  // Unknown witness programs are worth 0.
  return 0;
};

/**
 * Count the input cost for extended transaction inputs.
 * @param {Witness} witness
 * @returns {Number} input cost
 */

Script.prototype.getInputCost = function getInputCost(witness) {
  var factor = consensus.WITNESS_V9_FACTOR;
  var program;
  ...
  if (program.version === 9) {
    if (program.data.length === 20)
      return witness.getVarSize() * factor;

    if (program.data.length === 32)
      return witness.getVarSize() * factor;
  }

  // Unknown witness programs are worth 1.
  return witness.getVarSize() * 1;
};

/**
 * Calculate output cost for output script.
 * @returns {Number} output cost
 */

Script.prototype.getOutputCost = function getOutputCost() {
  var factor = consensus.WITNESS_V9_FACTOR;
  var program;

  // Exit
  if (!this.isExtension())
    return this.getVarSize() * factor;
...
Script.verifyProgram = function verifyProgram(witness, output, flags, tx, i, value) {
  ...
  if (program.version === 0 || program.version === 9) {
    if (program.data.length === 32) {
      if (stack.length === 0)
