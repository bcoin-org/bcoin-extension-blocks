...
MempoolEntry.prototype.fromTX = function fromTX(tx, view, height) {
  ...
  var extended = tx.isExtension(view);
  var fee = tx.getFee(view);
  var dependencies = false;
  var cost = 0;
  var size = 0;
  var i, input, sigops, priority;

  if (extended) {
    cost = tx.getExtensionCost(view);
    sigops = tx.getExtensionSigops(view, flags);
    size += tx.getSize();
    size += tx.getExitSize();
  } else {
    sigops = tx.getSigopsCost(view, flags);
    size += tx.getSigopsSize(sigops);
    size += tx.getEnterSize();
  }

  priority = tx.getPriority(view, height, size);
