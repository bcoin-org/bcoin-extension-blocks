...
Mempool.prototype.insertTX = co(function* insertTX(tx) {
  ...
  entry = MempoolEntry.fromTX(tx, view, this.chain.height, chain);

  // Verify that this is not redeeming
  // directly from an exited output.
  if (this.verifyExitInputs(entry)) {
    throw new VerifyError(tx,
      'invalid',
      'bad-txns-inputs-bad-exit-redemption',
      0);
  }
...
/**
 * Ensure a transaction is not _directly_
 * redeeming from an exit. They MUST redeem
 * from resolution TXs directly. Because of
 * how we store the UTXO set. This may look
 * valid unless we check for it.
 * @param {MempoolEntry} entry
 * @returns {Boolean}
 */

Mempool.prototype.verifyExitInputs = function verifyExitInputs(entry) {
  var tx = entry.tx;
  var i, input, coin, output;

  // Extended transactions are perfectly
  // allowed to redeem directly from
  // anything but main-chain outputs
  ...
  for (i = 0; i < tx.inputs.length; i++) {
    ...
    // We already checked for double spends.
    assert(!this.spents[input.prevout.hash]);

    coin = this.map[input.prevout.hash];

    // On-chain. Fine (we don't store exits
    // under their txid -- instead they're
    // under the resolution txid).
    if (!coin)
      continue;

    if (coin.extended) {
      output = coin.tx.outputs[input.prevout.index];

      // We already checked for orphans.
      assert(output);

      // We already checked the chain.
      assert(!output.script.isProgram());

      // We cannot redeem directly from an
      // exit's parent txid (it must be from
      // the resolution transaction id!).
      return false;
    }
  }
