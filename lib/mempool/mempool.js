...
Mempool.prototype._removeBlock = co(function* removeBlock(block, txs, ext) {
  ...
  for (i = 1; i < txs.length; i++) {
    ...
    // NOTE: Handle resolution TX.
    // Need a simple non-contextual way to do this.
    // Maybe a magic number pushdata?
    try {
...
Mempool.prototype.isEarlySpend = function isEarlySpend(tx) {
  var i, input, prevout;
