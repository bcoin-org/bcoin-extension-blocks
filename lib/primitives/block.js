...
Block.prototype.toNormal = function toNormal() {
  if (this.hasWitness() || this.hasExtension())
...
Block.prototype.frame = function frame() {
  ...
  if (this._raw) {
    ...
    raw = new RawBlock(this._size, this._witness, this._extension);
    raw.data = this._raw;
    return raw;
  }
...
/**
 * Test whether the block has an exiting output.
 * @returns {Boolean}
 */

Block.prototype.hasExit = function hasExit() {
  var i, j, tx, output;
...
Block.prototype.createExtensionHash = function createExtensionHash(enc) {
  var leaves = [];
  var i, tx, root, hash;
...
Block.prototype.getExtensionHash = function getExtensionHash(enc) {
  var i, coinbase, hash, output;
  ...
  for (i = coinbase.outputs.length - 1; i >= 0; i--) {
    output = coinbase.outputs[i];
    if (output.script.isExtensionHash()) {
      hash = output.script.getExtensionHash();
      break;
    }
  }
...
Block.prototype.verifyBody = function verifyBody(ret) {
  ...
  for (i = 0; i < this.ext.length; i++) {
    ...
    // Outputs must be witness programs.
    if (!tx.verifyExits()) {
...
Block.prototype.fromReader = function fromReader(br) {
  ...
  if (!this.mutable) {
    ...
    this._size = this._raw.length - extension;
...
Block.prototype.frameWitness = function frameWitness() {
  ...
  var bw = new StaticWriter(sizes.total + sizes.extension);
...
Block.prototype.getWitnessSizes = function getWitnessSizes() {
  ...
  if (this.ext.length > 0) {
    ...
    for (i = 0; i < this.ext.length; i++) {
      tx = this.ext[i];
      sizes = tx.getSizes();
      extension += sizes.total;
    }
  }

  return new RawBlock(size, witness, extension);
};
...
function RawBlock(total, witness, extension) {
  this.data = null;
  this.total = total;
