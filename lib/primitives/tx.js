 ...
 * @param {CoinView} view
 * @returns {Number} sigops cost
 */

TX.prototype.getExtensionSigops = function getExtensionSigops(view) {
  var cost = 0;
  var i, input, coin;
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    cost += coin.script.getExtensionSigops(input.witness);
  }
...
TX.prototype.verifyExits = function verifyExits() {
  ...
  for (i = 0; i < this.outputs.length; i++) {
    ...
    // Non-witness-programs are considered
...
TX.prototype.getSafeFee = function getSafeFee(view) {
  var total = 0;
  var value = 0;
  var i, input, coin, fee;
...
TX.prototype.hasDust = function hasDust() {
  ...
  for (i = 0; i < this.outputs.length; i++) {
    ...
    if (output.value < 500 && output.script.isExtension())
      return true;
  }
...
TX.prototype.checkContext = function checkContext(view, height, ret) {
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    if (coins.resolution) {
      ...
      // Other outputs need a maturity level.
      if (height - coins.height < consensus.RESOLUTION_MATURITY) {
        ret.reason = 'bad-txns-premature-spend-of-resolution';
        ret.score = 0;
        return -1;
      }
    }
