...
TX.prototype.getSigopsSize = function getSigopsSize(sigops) {
  ...
  var weight = Math.max(this.getSize() * scale, sigops * bytes);
...
TX.prototype.hasCanonical = function hasCanonical(view) {
  var i, input, coin;
...
TX.prototype.hasPrograms = function hasPrograms(view) {
  var i, input, coin;
...
TX.prototype.checkInputs = function checkInputs(view, height, resolution, genesis, ret) {
  return this.checkContext(view, height, resolution, genesis, ret) !== -1;
};
...
TX.prototype.checkContext = function checkContext(view, height, resolution, genesis, ret) {
  var total = 0;
  var value = 0;
  var i, input, output, coins, coin, fee;
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    if (resolution) {
      if (i === 0 && !genesis) {
        // First input must be another resolution output.
        // TODO: Account for genesis resolution.
        if (!coins.resolution || input.prevout.index !== 0) {
          ret.reason = 'bad-txns-inputs-resolution';
          ret.score = 100;
          return -1;
        }

        assert(coin.script.isResolution());
      } else {
        // Exiting outputs cannot directly re-enter.
        if (coins.resolution) {
          ret.reason = 'bad-txns-inputs-entrance';
          ret.score = 100;
          return -1;
        }

        // Non-program outputs cannot enter.
        if (!coin.script.isProgram()) {
          ret.reason = 'bad-txns-inputs-entrance';
          ret.score = 100;
          return -1;
  ...
  for (i = 0; i < this.outputs.length; i++) {
    ...
    if (i === 0) {
      // First output script must
      // be a resolution output.
      if (!output.script.isResolution()) {
        ret.reason = 'bad-txns-outputs-resolution';
        ret.score = 100;
        return -1;
      }
    } else {
      // Program outputs cannot exit.
      if (output.script.isProgram()) {
        ret.reason = 'bad-txns-outputs-exit';
        ret.score = 100;
        return -1;
      }
    }
...
TX.prototype.fromWitnessReader = function fromWitnessReader(br) {
  ...
  for (i = 0; i < count; i++) {
    ...
    assert(input.script.getSize() === 0,
      'Non-zero script in witness transaction.');
