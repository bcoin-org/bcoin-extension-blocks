...
Coin.prototype.fromJSON = function fromJSON(json) {
  ...
  assert(typeof json.extended === 'boolean', 'Extended must be a boolean.');
  assert(typeof json.resolution === 'boolean', 'Resolution must be a boolean.');
...
Coin.prototype.getSize = function getSize() {
  return 19 + this.script.getVarSize();
};
...
Coin.prototype.fromTX = function fromTX(tx, index, height, extended) {
  ...
  this.index = index;
  // NOTE: NOT CONSENSUS SAFE:
  this.extended = this.script.isProgram();
  this.resolution = tx.isResolution();
  return this;
};
...
Coin.fromTX = function fromTX(tx, index, height, extended) {
  return new Coin().fromTX(tx, index, height, extended);
};
