...
MTX.prototype.addTX = function addTX(tx, index, height, extended) {
  var input, coin;
  ...
  coin = Coin.fromTX(tx, index, height, extended);
...
MTX.prototype.signInput = function signInput(index, coin, ring, type) {
  ...
  if (prev.isScripthash()) {
    ...
    redeem = true;
  } else if (prev.isWitnessScripthash()) {
...
CoinSelector.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.chain != null) {
    assert(typeof options.chain === 'number');
    assert(options.chain >= -1 && options.chain <= 1);
    this.chain = options.chain;
  }

  if (options.extension != null) {
    assert(typeof options.extension === 'boolean');
    this.chain = +options.extension;
  }
...
CoinSelector.prototype.init = function init(coins) {
  this.coins = this.selectChain(coins);
...
CoinSelector.prototype.selectChain = function selectChain(coins) {
  var result = [];
  var main = 0;
  var extension = 0;
  ...
  if (chain === -1) {
    for (i = 0; i < coins.length; i++) {
      ...
      if (coin.script.isProgram())
        extension += coin.value;
      else
        main += coin.value;
    }

    if (extension >= main && this.tx.verifyExits())
      chain = 1;
    else
      chain = 0;
  }
...
CoinSelector.prototype.isSpendable = function isSpendable(coin) {
  ...
  if (coin.resolution) {
    ...
    if (this.height + 1 < coin.height + consensus.RESOLUTION_MATURITY)
      return false;
